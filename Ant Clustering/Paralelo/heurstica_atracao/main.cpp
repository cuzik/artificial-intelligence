...
int main(int argc, char const *argv[]){
    ...
    /*Desenha o estado inicial do formigueiro*/
    ...
    int rc;
    for(i=0;i<ant_alive;i++){
        rc = pthread_create(&cons[i], NULL, ant_animated,(void *)i);
    }
    ...
    while(!sair){
        ...
        frames++;
        al_draw_textf(fonte, al_map_rgb(255, 255, 255), 10 , 0, 0, "Frames:%lli range:%i", frames*taxa_atualizacao_tela,ants[0].vision);
        al_flip_display();
    }
